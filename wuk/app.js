...
function render(){
 ...
 list.forEach(s=>{let d=document.createElement("article");d.className="card";d.tabIndex=0;d.setAttribute("role","button");d.setAttribute("aria-label",`Preview ${s.id}: ${s.title}`);d.dataset.id=s.id;d.innerHTML=`<div class="scenario-copy"><div class="id">${esc(s.id)}</div><div class="title">${esc(s.title)}</div><div class="loc">${esc(s.location||"")}</div>${dots(s.complexity)}<div class="tags">${(s.focus||[]).map(t=>`<span class="tag">${esc(t)}</span>`).join("")}</div><div class="tags comp" style="margin-top:7px">${(s.competencies||[]).map(t=>`<span class="tag">${esc(t)}</span>`).join("")}</div><span class="open">Open preview <span aria-hidden="true">→</span></span></div>${s.id==="LL001"?'<div class="scenario-route" aria-hidden="true"><img src="assets/ll001-south-coast.svg" alt=""><span>LTN / RWY 07</span></div>':""}`;results.appendChild(d)});
}
