...
const resend=document.getElementById('requestSetupForm');resend.onsubmit=async event=>{event.preventDefault();const button=resend.querySelector('button');button.disabled=true;try{const response=await fetch(AUTH+'/recover?redirect_to='+encodeURIComponent('https://linelogic.uk/?home=1'),{method:'POST',headers:{apikey:KEY,'Content-Type':'application/json'},body:JSON.stringify({email:document.getElementById('requestSetupEmail').value.trim()})});if(!response.ok)throw new Error('Unable to send a new link. Please wait a moment and try again.');message.textContent='Check your inbox for a fresh setup link. Use the newest email.';}catch(error){message.textContent=error.message;}finally{button.disabled=false;}};
})();
